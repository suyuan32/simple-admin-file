...
func (l *UploadLogic) Upload() (resp *types.UploadResp, err error) {
	...
	// generate path
	// 生成路径
	err = os.MkdirAll(path.Join(l.svcCtx.Config.UploadConf.StorePath,
		l.svcCtx.Config.Name, fileType, timeString), 775)
	if err != nil {
		return nil, errorx.NewApiErrorWithoutMsg(http.StatusInternalServerError)
	}

	targetFile, err := os.Create(path.Join(l.svcCtx.Config.UploadConf.StorePath, l.svcCtx.Config.Name,
